...
impl Ext4Filesystem {
    ...
    pub fn new(path: &str) -> Result<Self, Ext4Error> {
        ...
        for i in 0..block_groups_count {
            let mut file_clone = file.try_clone()?;
            let block_group = BlockGroup::read(&mut file_clone, i, superblock.first_data_block, block_size)?;
    ...
    pub fn read_inode(&mut self, inode_num: u32) -> Result<Inode, Ext4Error> {
        if inode_num == 0 || inode_num > self.superblock.inodes_count {
            return Err(Ext4Error::InvalidInode(format!("Invalid inode number: {}", inode_num)));
        }

        let group_idx = (inode_num - 1) / self.superblock.inodes_per_group;
        if group_idx as usize >= self.block_groups.len() {
            return Err(Ext4Error::InvalidInode(format!("Invalid block group index: {}", group_idx)));
        }
    ...
    pub fn read_directory(&mut self, inode_num: u32) -> Result<Directory, Ext4Error> {
        ...
        if !inode.is_directory() {
            return Err(Ext4Error::InvalidDirectory(format!("Inode {} is not a directory", inode_num)));
        }
    ...
    pub fn open_file(&mut self, inode_num: u32) -> Result<File, Ext4Error> {
        let inode = self.read_inode(inode_num)?;
        if !inode.is_file() {
            return Err(Ext4Error::InvalidFile(format!("Inode {} is not a regular file", inode_num)));
        }
    ...
    pub fn read_file(&mut self, inode_num: u32, buffer: &mut [u8], position: u64) -> Result<usize, Ext4Error> {
    ...
    pub fn find_by_path(&mut self, path: &str) -> Result<u32, Ext4Error> {
        ...
        for component in components {
            ...
            match directory.find_entry(component) {
                ...
                None => {
                    return Err(Ext4Error::InvalidFile(format!("Path component not found: {}", component)));
                }
            }
        }
    ...
    pub fn write_file(&mut self, parent_path: &str, filename: &str, data: &[u8]) -> Result<(), Ext4Error> {
        ...
        if !parent_inode.is_directory() {
            return Err(Ext4Error::InvalidDirectory(format!("'{}' is not a directory", parent_path)));
        }
        ...
        let inode_num = match existing_entry {
            Some(entry) => {
                ...
                if !inode.is_file() {
                    return Err(Ext4Error::InvalidFile(format!("'{}' exists but is not a regular file", filename)));
                }
        ...
        if blocks_needed > 12 {
            return Err(Ext4Error::InvalidOperation("Files larger than 12 direct blocks are not supported yet".to_string()));
        }
        ...
        if existing_entry.is_none() {
            self.add_directory_entry(parent_inode_num, filename, inode_num, 1)?; // 1 = regular file
        }
    ...
    pub fn remove_file(&mut self, path: &str) -> Result<(), Ext4Error> {
        ...
        if !inode.is_file() {
            return Err(Ext4Error::InvalidFile(format!("'{}' is not a regular file", path)));
        }
    ...
    pub fn remove_directory(&mut self, path: &str, force: bool) -> Result<(), Ext4Error> {
        ...
        if !inode.is_directory() {
            return Err(Ext4Error::InvalidDirectory(format!("'{}' is not a directory", path)));
        }
        ...
        if !force {
            ...
            let real_entries = directory.entries.iter()
                .filter(|entry| entry.name != "." && entry.name != "..")
                .count();
                
            if real_entries > 0 {
                return Err(Ext4Error::InvalidOperation(
                    format!("Directory '{}' is not empty. Use force flag to remove anyway.", path)
                ));
            }
        }
    ...
    pub fn create_directory(&mut self, parent_path: &str, dirname: &str) -> Result<(), Ext4Error> {
        ...
        if directory.find_entry(dirname).is_some() {
            return Err(Ext4Error::InvalidOperation(format!("Directory '{}' already exists", dirname)));
        }
    ...
    fn allocate_inode(&mut self) -> Result<u32, Ext4Error> {
        ...
        for (group_idx, block_group) in self.block_groups.iter().enumerate() {
            ...
            for byte_idx in 0..block_size as usize {
                if bitmap[byte_idx] != 0xFF {  // If not all bits are set
                    for bit_idx in 0..8 {
                    ...
                    for bit_idx in 0..8 {
                        if (bitmap[byte_idx] & (1 << bit_idx)) == 0 {
                            ...
                            if block_idx < self.superblock.blocks_per_group as usize {
                                ...
                                let block_num = group_idx as u32 * self.superblock.blocks_per_group + block_idx as u32 + 
                                    (if group_idx == 0 { self.superblock.first_data_block } else { 0 });
    ...
    fn free_inode(&mut self, inode_num: u32) -> Result<(), Ext4Error> {
        ...
        // In a real implementation, you would update the inode bitmap
        Err(Ext4Error::InvalidOperation("Inode freeing not implemented".to_string()))
    }
    
    /// Free a block.
    fn free_block(&mut self, block_num: u32) -> Result<(), Ext4Error> {
        // For simplicity, do nothing for now
        // In a real implementation, you would update the block bitmap
        Err(Ext4Error::InvalidOperation("Block freeing not implemented".to_string()))
    }
    
    /// Add an entry to a directory.
    fn add_directory_entry(&mut self, dir_inode_num: u32, name: &str, inode_num: u32, file_type: u8) -> Result<(), Ext4Error> {
        // For simplicity, do nothing for now
        // In a real implementation, you would update the directory data
        Err(Ext4Error::InvalidOperation("Directory entry addition not implemented".to_string()))
    }
    ...
    fn remove_directory_entry(&mut self, dir_inode_num: u32, name: &str) -> Result<(), Ext4Error> {
        // For simplicity, do nothing for now
        // In a real implementation, you would update the directory data
        Err(Ext4Error::InvalidOperation("Directory entry removal not implemented".to_string()))
    }
    ...
    fn write_directory_entries(&mut self, block_num: u32, dir_inode_num: u32, parent_inode_num: u32) -> Result<(), Ext4Error> {
        let block_size = self.superblock.block_size();
    ...
    fn write_superblock(&mut self) -> Result<(), Ext4Error> {
        ...
        if self.superblock.rev_level >= 1 {
            ...
            while bg_idx < self.block_groups.len() as u32 {
                let offset = bg_idx * self.superblock.blocks_per_group * self.superblock.block_size() + 1024;
                file_clone.seek(SeekFrom::Start(offset as u64))?;
